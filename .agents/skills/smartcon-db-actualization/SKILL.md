...
| Задачи | та же папка: `HashFormatActualizationTask` (`hash-v6`, Order=10, critical, override'ит `ApplyAsync`/`HandleGroupFailureAsync`), `AttributesActualizationTask` (`attributes-v1`, 20, optional), `GlbPreviewActualizationTask` (`glb-v1`, 30, optional), `RevitCategoryActualizationTask` (`revit-category-v1`, 40, optional), `FamilyFactsActualizationTask` (`family-facts-v1`, 50, optional, детект генерируется из `FamilyFactRuleSet` — ADR-055), `MiniProjectMarkerActualizationTask` (`mini-project-marker-v1`, 60, optional — **пишет В MANAGED-ФАЙЛ**, см. секцию ниже) |
