...
## Ранбук итераций (жёстко — экономит 20+ минут на цикл)

**Инцидент 2026-08-28:** агент прогнал полный сьют (~5 мин), затем прогнал
его ЕЩЁ РАЗ только чтобы узнать имя упавшего теста — и не получил его из-за
кривого grep. Пользователь ждал ~20 минут впустую. Правила ниже — чтобы
...
2. **Правильные `--treenode-filter` паттерны** (проверено прогонами;
   ...
   - класс: `--treenode-filter "/*/*/*ClassName*/*"`
   - метод: `--treenode-filter "/*/*/*/*MethodName*"`
   - Паттерн с именем класса во ВТОРОМ сегменте (`/*/*ClassName*/*/*`)
     молча даёт **«Запущено ноль тестов» за 72 мс** — ВСЕГДА проверяй
     счётчик `всего: N` > 0, прежде чем считать прогон валидным.
3. **НИКОГДА не перезапускай прогон ради имён упавших.** Имена и причины
   извлекай из ЭТОГО ЖЕ запуска:
   - консоль: ищи `сбой TestName (XXms)` и стектрейс
     `TestFailedException ... в SmartCon.IntegrationTests.<Файл>:строка`;
   - либо добавь `--report-trx` и grep-ай TRX (имена + message в XML);
   - либо парси `bin/<cfg>/TestResults/*-report.html` (данные встроены).
4. **Скипы — часть результата.** Счётчики `пропущено: N` сравнивай с
   baseline прошлого прогона: новые скипы = такой же сигнал регрессии,
   как падения (skip-гард мог сработать на твоём изменении).
