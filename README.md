...
|---------|----------|-------|
| **PipeConnect** | Соединение MEP-элементов двумя кликами на любом виде | [YouTube](https://www.youtube.com/watch?v=F55N9cCMIMk) |
| **Share Project** | Экспорт модели в Shared-зону (ISO 19650) | — |
| **FamilyManager** | Управление библиотекой семейств Revit: 3D-просмотр, версии, дедупликация, пакетный импорт | — |

---

<div align="center">

### PipeConnect — Мечта всех MEP инженеров в Revit (работает на всех видах)
...
Флагманский модуль SmartCon. Соединяет MEP-трубопроводы двумя кликами **на любом виде** (3D, планы, разрезы, фасады) — с автоматическим подбором фитингов, переходников и типов соединений.
...
```
1. Откройте любой вид в Revit (3D, план, разрез, фасад)
...
Импорт, каталогизация, 3D-просмотр, версионность, поиск и загрузка семейств в проект — всё из одного окна.

### FamilyManager за 30 секунд

```
1. Нажмите FamilyManager на панели Family Library
2. При первом запуске — создайте каталог (укажите папку)
3. Импортируйте семейства: пакетный импорт .rfa, импорт активного проекта или выделенных элементов
4. Семейства появятся в каталоге с автоматическим извлечением типов, атрибутов и 3D-геометрии
5. Перетащите семейство мышкой прямо в окно Revit — оно разместится автоматически
6. Или выберите семейство → Properties для 3D-просмотра, версий и атрибутов
```
...
|---|---|
| **3D-просмотр** | Интерактивный вьювер семейства с вращением, зумом и переключением типов — работает на Revit 2019–2025+ |
| **Управление версиями** | Вкладка «Версии»: история всех версий с автором и датой, переключение активной версии одним кликом, откат к любой версии |
| **Дедупликация по содержимому** | Автоматическое обнаружение дубликатов, даже переименованных или пересохранённых через «Save As» |
| **Пакетный импорт** | Выбор нескольких `.rfa` за раз, режимы «Новая версия / Перезаписать / Пропустить», мульти-выбор строк |
| **Drag & Drop в проект** | Перетаскивание семейства из каталога прямо в окно Revit — автоматическое размещение |
| **Индикатор устаревших семейств** | Оранжевый маркер для семейств, загруженных в проект старой версией |
| **Импорт системных семейств** | Трубы, гибкие трубы, воздуховоды, короба, лотки, стены — прямо из активного проекта |
| **Type Catalog bake-in** | Каталоги типоразмеров «запекаются» в `.rfa` при импорте — без `.txt`-сайдкаров |
| **Импорт активного файла** | Быстрый анализ (<1 сек на 200 семейств), импорт только размещённых семейств |
| **Поиск по каталогу** | Автораскрытие категорий, подсветка совпадений, «Развернуть всё / Свернуть всё» |
| **Найти в каталоге** | Подсветка семейства из активного выделения Revit с автопрокруткой |
| **Редактировать в Family Editor** | Открытие `.rfa` прямо из каталога, не выходя из SmartCon |
| **Единый диалог вложенных семейств** | Одно окно для всех сценариев загрузки общих вложенных семейств |
...
После установки на ленте Revit появляется вкладка **SmartCon** с четырьмя панелями:

### Pipe Systems

| Кнопка | Назначение |
|--------|-----------|
| **PipeConnect** | Соединение MEP-элементов двумя кликами на любом виде |
...
SmartCon.Tests             — Unit-тесты (xUnit + Moq, 1500+ тестов)
```
