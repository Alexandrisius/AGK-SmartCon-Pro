...
[S6: PostProcessing] -- Открывается PipeConnectEditor (немодальное окно)
  ...
  |   |-- S6.1: Проверить CTC нового коннектора (пользователь видит какой коннектор назначает)
  |   |   |-- CTC известен (в маппинге или VirtualCtcStore) --> продолжить
  |   |   |-- CTC отсутствует --> MiniTypeSelector поверх окна
  |   |       |-- Пользователь выбрал тип --> записать в VirtualCtcStore / Description --> продолжить
  |   |       |-- Пользователь отменил --> RollbackCycleAlignment + RestoreChainLevels + UnmarkVisited
  |   |-- Установить _chainDisabledByCycle по результату IsChainConnector:
  |   |   |-- Целевой коннектор = цепочечный --> _chainDisabledByCycle = true (кнопки сети скрыты)
  |   |   |-- Целевой коннектор ≠ цепочечный --> _chainDisabledByCycle = false (кнопки сети доступны)
  |   |-- S6.2: ReevaluateAfterCycle
  |   |   |-- S5-re: пересчёт фитингов для новой пары CTC (static <-> new dynamic)
  |   |   |-- Удалить старый фитинг/reducer (если были)
  |   |   |-- Вставить новый фитинг (первый из пересчитанного списка)
  |   |   |-- S4-re: перезагрузка AvailableDynamicSizes для нового коннектора
  |   |   |-- Проверка необходимости reducer (сравнение радиусов)
