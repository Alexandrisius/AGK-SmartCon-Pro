...
| Кнопка + | Button -> ICommand | `IncrementChainDepthCommand` | Подключить следующий элемент очереди (ровно один). На границе уровня — авто-seal: тихий остаток сети переподключается одной транзакцией (ADR-052) |
| «Подключить всё» | Button -> ICommand | `ConnectAllChainCommand` | Fast path: тихий остаток — seal одной транзакцией; иначе поэлементный обход. **Доступна на любом шаге**, пока есть неподключённые элементы и цепь не запечатана |
...
(`UnsealIfSealed`), затем выполняет операцию — seal экономит обход тихих сетей,
но никогда не мешает редактированию.
