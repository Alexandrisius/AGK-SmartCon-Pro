...
## Очистка недоступных записей (#133)

Постоянная команда «Очистить недоступные записи» в popup инструментов базы
(видна при активной БД и роли Owner/BimMaster) — гигиена файловой системы,
НЕ актуализация: миграции запускаются только для реальных миграций данных.

- **Сканер на живой базе**: открытие команды запускает скан — дешёвый SQL по
  маркеру `hash_format_version = -2` ПЛЮС полный проход `File.Exists` по всем
  managed-файлам (главный детектор файла, удалённого вручную при запущенном
  Revit — без закрытия Revit и без миграций). Кандидаты приходят
  инкрементально через `MissingRecordScanProgress.Found` — прерванный скан
  сохраняет найденное; лейбл кандидат только если отсутствуют ВСЕ его варианты.
- **Диалог** (modeless, ADR-048): прогресс-бар «Проверка X из Y — файл»,
  «Прервать», чек-боксы, «Удалить выбранные/все» с подтверждением и
  предупреждением о временно недоступных сетевых дисках.
- **Purge** — `ICatalogActualizationService.PurgeMissingAsync`: удаление
  версии НЕ удаляет семейство, пока живы другие версии (активная
  переключается автоматически и сообщается); мусорный фитинг удаляется со
  сбросом routing-связей родителей (см. ADR-067 as-built) и предупреждением
  о stale-трассировке в проекте.
- **Routing-фантомы**: правила трассировки хранят фитинг строкой «Family:Type»
  без FK — после purge правило остаётся с мёртвой ссылкой.
  `IRoutingEditorService.FindRoutingPhantomsAsync` (чистый проход по каталогу,
  после каждой загрузки дерева) помечает затронутые семейства значком
  (SourceBranch); клик открывает свойства сразу на вкладке «Трассировка» на
  затронутом типе — замена фитинга гасит фантом.
...
Отклонения — через `override` virtual-членов базы (образцы: `HashFormatActualizationTask`
переопределяет `ApplyAsync` (пересчёт + ресинк item + bump `min_plugin_version`) и
`HandleGroupFailureAsync` (терминальные маркеры -1/-2); `MiniProjectMarkerActualizationTask`
переопределяет `RequiresExtraction`→false — file-level задача, движок не открывает файл
группы вообще). Полный шаблон: навык
...
- [ADR-065](../adr/065-fhv4-extended-sync.md) — формат хэша v6 (FHV6, задача `hash-v6`; ранее FHV5/`hash-v5`, FHV4/`hash-v4`) + расширенный sync (подтипы лестниц, структура ограждений, настройки провода)
