...
> Загружать: при создании/перемещении файлов или при вопросах «куда положить код?»

## Правило размера файлов (#260)

Production-файлы `.cs`: **цель ≤500 строк, жёсткий лимит 600**. Превышение —
разбивка на partial-файлы по зонам ответственности с каноном именования
`Class.Topic.cs` (`FamilyManagerMainViewModel.Tree.cs`, `LocalCatalogMigrator.MigrationsV15toV22.cs`).
Поля (включая инициализаторы), primary constructor и список базовых типов
остаются в «ядровом» файле; `#if`-блоки переносить только целиком.
Исключения (data-словари локализации, неделимые single-методы) — см. Issue #260.

Деревья ниже — обзорная карта (не исчерпывающий список файлов): крупные классы
разбиты на partial-файлы в тех же папках.
