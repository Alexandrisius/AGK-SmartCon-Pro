...
- **Известные ограничения canonical string (приняты, формат заморожен после миграции ADR-050):**
  1. Разделитель `|` в значениях полей не экранируется — теоретическая коллизия при специально подобранных значениях (`X|Y` в одном поле против `X`+`Y` в соседних). Последствие — ложный дубликат, обнаружимый пользователем; вероятность ничтожна. Экранирование меняет формат → новая миграция; отложено до будущей v3 (паттерн `docs/architecture/database-migrations.md` это покрывает).
  2. `IsBlankValue` трактует строковые значения `"INVALID"`/`"UNSUPPORTED"`/`"READERROR"` как пустые (наследие маркеров экстрактора) — параметр с таким буквальным значением не влияет на хэш. Тот же порядок риска, тот же путь исправления (v3).
