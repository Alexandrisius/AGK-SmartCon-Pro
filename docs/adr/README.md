...
| [072](072-manual-miniproject-staging.md) | Ручной staging мини-проектов MEPCurve + routing как данные каталога (#254) — отказ от CopyElements в staging (дубли материалов Revit 2024+), мини без фитингов, таблица family_routing_rules, редактор трассировки из каталога; полная криминалистика #254 | accepted (amended by 073) | 2026-08-28 |
| [073](073-fhv21-versioned-segment-config.md) | FHV21 - сегментная конфигурация трубы (набор + диапазоны + порядок) как версионный контент мини-проекта: диапазоны в хэше (FHV11), per-version таблица family_segment_rules (V38), откат восстанавливает конфигурацию версии, строка Segments read-only, фильтры пикера по connector_shape (amends ADR-072) | accepted | 2026-09-01 |
| [074](074-catalog-compliance-check.md) | Комплаенс-проверка каталога («Проверить → Правила», #259) — две независимые команды вместо расширения StaleReason: каталог vs effective-правила категории (pure SQLite, без открытого проекта), ICatalogComplianceService + сессионный снимок без миграции схемы, красный щит Fail / оранжевый треугольник CannotVerify по канону #210, отчёт через существующий ValidationReportViewModel | accepted | 2026-09-08 |
| [075](075-cloud-catalog-architecture.md) | Cloud Catalog Architecture — publish/subscribe sync поверх локальных каталогов: publish points, CAS (SHA-256), раздельная доставка API/файлы (tunnel + R2 presigned), стек ASP.NET Core 10 / PostgreSQL 18 / R2 (MinIO запрещён) | proposed | 2026-08-26 |
| [076](076-cloud-catalog-security.md) | Cloud Security & Credentials — облачные аккаунты (Identity), JWT per-request authz, ключи SCCAT (SHA-256, maxActivations=1, аудит), Credential Manager, квоты upload, ФЗ-152 (soft-delete, consent, шифрованные бэкапы) | proposed | 2026-08-26 |
| [077](077-cloud-catalog-multi-author-sync.md) | Cloud Multi-Author Sync — pull-before-push, per-item optimistic concurrency, rehash hash-epoch, иммутабельность опубликованных версий, store delete-guard | proposed | 2026-08-26 |
