...
**Date:** 2026-08-28
**Status:** accepted (владелец 2026-08-29: «реализовать»; Фазы 0, 1+2, 2b РЕАЛИЗОВАНЫ 2026-08-29, Фаза 3 РЕАЛИЗОВАНА 2026-08-29 — см. «Статус реализации» ниже; остались Ф4 non-MEP, Ф5 UX-гейт — отложены владельцем как второстепенные)
...
> - **Фаза 3 — DONE** (2026-08-29). Реализация:
...
>     диалог Revit, §2.7 п.4); `FamilyContentHasher.RebuildSystemSectionsWithRouting`
>     + `FormatSystemRoutingSection` — пересчёт хэшей/секций из БД без Revit,
>     byte-exactness со snapshot-хэшером доказана юнит-тестами против
>     реального пути хэширования.
>   - **Движок:** `IRoutingEditorService`/`CatalogRoutingEditorService` —
>     сохранение правки = новая версия одной транзакцией (клон версии +
>     family_types + routing V34 + хэши + секции + указатель + links;
>     glb_state сбрасывается для перегенерации превью; routing_backfilled=1).
>     Legacy-версии без section_strings отклоняются с подсказкой. Links:
>     shared_nested переносятся, routing пересобираются (динамические
>     link-индикаторы, D1). Archived-lock: убранные детали, живущие в
>     архивных версиях, возвращаются как UX-подсказка (ADR-067).
>   - **UI:** вкладка «Трассировка» в свойствах итема (только system MEP):
>     per-type селектор, preferred junction (Tee=0/Tap=1), группы по
>     категории, multi-rule с критериями мин./макс. (мм, «Все») и порядком
>     ↑↓, param-группы — одиночные строки, Segments read-only, presence-флаги
>     ⚠ «нет в каталоге», пикер «семейство+ТИП из всего каталога» с фильтром
>     категория+part_type и поиском. Сохранение интегрировано в общий Save
>     свойств; после сохранения дерево каталога перечитывается
>     (VersionsChanged → LoadTreeAsync → свежие dependency-индикаторы).
>     Per-type хэши новой версии пересчитаны → stale-точки #253 корректны.
>   - **Гейты:** unit 3069/3069 (+54 к пре-Ф3: byte-exactness recompose,
>     матрица групп, save-pipeline SQLite, edit-state, коллизия имён типов,
>     размеры сегментов); builds R25/R21 0/0.
>     Интеграционный сьют не затронут (Ф3 — DB-only + UI; ручной тест
>     владельцем обязателен перед релизом).
>   - **Кейс коллизии имён типов (валидатор Ф3, major M1 — закрыт):**
>     у воздуховодов три системных семьи (Round/Rect/Oval, FHV7) и у коробов
>     две (With/WithoutFittings) — одноимённые типы разных семей в одном
>     итеме коллайдят в name-ключе секций V33 (`"{SECTION}|{typeName}"` —
>     существующее ограничение формата, не введённое Ф3). Байт-экзактная
>     рекомпозиция для таких итемов невозможна: редактор показывает их
>     read-only с подсказкой «переимпортируйте категорию», а
>     `SaveAsync` отказывает кодом `TypeNameCollisions` до любой записи
>     (покрыто 2 тестами). Системное решение — семейный ключ в ключе секции —
>     требует миграции хранилища V33 и вынесено отдельной задачей.
>   - **Размеры правил как в Revit (требование владельца):** V36
>     `family_segment_sizes` — per-version таблицы размеров сегментов;
>     dropdown'ы мин./макс. предлагают NominalDiameter («Все» = сентинел
>     0..10000 ft ревитовского «all sizes», раундтрип байт-в-байт). Пишется
>     при импорте (`SegmentSizeWriter` в обоих batch executors), копируется
>     при save редактора, backfill legacy — optional-задача
>     `segment-sizes-v1` (детект: системные трубы без строк размеров).
>     Нет размеров (воздуховоды/короба/лотки) — fallback free-text.
