...
- Защита от одновременной записи: один writer (SQLite DELETE journal, I-14).
  WAL рассматривался и отклонён (ADR-050) — не работает поверх сетевых ФС (SMB)
...
- DELETE journal + один writer + `Mode=ReadOnly` для read-only ролей — корректно работает по SMB (WAL поверх SMB не работает и отклонён, ADR-050)
