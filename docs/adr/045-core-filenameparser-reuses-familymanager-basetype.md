...
## Update A4 — несохранённый документ блокирует проектные базы (2026-07-28, #174)

Ранее (решение «ignore unsaved» из #119) `ActiveDocumentChangeNotifier`
игнорировал документы с пустым `PathName`: переключение на новый
несохранённый файл («Проект1») было невидимым, и проектная база оставалась
активной со stale-путём предыдущего документа — пользователь мог загружать
семейства из проектной базы в файл, который «ещё не существует официально».

Пересмотрено:

- **Нотификатор** больше не игнорирует несохранённые документы: поднимает
  `ActiveDocumentChanged` с **пустым путём** (семейные документы и
  intra-document переключение видов по-прежнему фильтруются).
- **Активатор** на пустом пути не скипает, а применяет fallback «нет
  совпадений»: активная общая — сохранить; активная проектная/нет активной —
  переключить на первую общую базу (если есть).
- **VM** (`RecomputeActiveBaseMatch`): гарантированно полученный пустой путь
  + активная проектная база → `_activeBaseCompatibleWithCurrentDoc = false`
  (загрузка заблокирована) + статус «Файл не сохранён — проектные базы
  недоступны» вместо permissive default `true`. Срабатывает и при ручном
  выборе проектной базы в ComboBox на несохранённом документе, не только
  после авто-фоллбэка. **Важно:** `null` («событие ещё не получали» — напр.
  race старта Revit двойным кликом по .rvt, когда `ViewActivated` стреляет
  до `Register`) НЕ трактуется как unsaved: контекст неизвестен → legacy
  permissive без блокировки; блокировка только по позитивно полученному
  пустому пути.
- Дополнительно в нотификаторе: переход на несохранённый документ сбрасывает
  `_lastNotifiedPath` — иначе dedup проглатывал легитимный первый Save/SaveAs,
  если новый путь совпадал с путём ранее оповещённого документа (Save As
  поверх того же файла). Проверка «previous == current» (intra-document
  переключение видов) вынесена перед unsaved-веткой — повторный
  empty-notify при переключении видов внутри несохранённого документа не
  поднимается.
- **Сохранение файла** по-прежнему запускает оценку шаблона (Update A2:
  `DocumentSaved`/`DocumentSavedAs` → `ActiveDocumentPathChanged`).

Заодно: команда «Настроить правила проектной базы» получила контекстную
видимость (скрыта для общих баз) — тот же паттерн, что у команд
конвертации (Update A3).
