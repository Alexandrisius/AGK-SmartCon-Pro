...
Одна строка (файл) в диалоге пакетного импорта. Содержит метаданные файла, статус и выбранное пользователем действие. `IsMarkerResolvedVersion` (#180, 2026-08-12): matched-версия резолвлена верифицированным ES-маркером, а не контент-хэшем — UI показывает суффикс «— маркер» и тултип; импорт-логика флаг не потребляет (MakeActive/executor читают только label).
