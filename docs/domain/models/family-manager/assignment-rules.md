...
(ordinal категории, facts, имя семейства, FamilyKey).
...
`Ambiguous(candidateIds)`. Несколько подошедших категорий — это конфликт,
который разруливает пользователь через рекомендательный пикер (иконка ⚠
в колонке «Категория» batch-диалога); движок намеренно НЕ выбирает
победителя по приоритету.
