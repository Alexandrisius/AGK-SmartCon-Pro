...
**Удалён** (ADR-024 cleanup). JSON-экспорт/импорт метаданных каталога теперь живёт напрямую в `CategoryTreeEditorViewModel.ExportImport.cs` и работает через прямые вызовы `ICategoryRepository`, `IAttributeDefinitionRepository`, `ICategoryAttributeBindingService`. DTO для JSON-обмена — `SmartCon.FamilyManager/Models/Metadata/MetadataExportPackage.cs`.
...
## RBAC Interfaces *(FamilyManager)*

Интерфейсы системы Role-Based Access Control для локальных каталогов.
...
## Drag & Drop Contracts *(FamilyManager)*
