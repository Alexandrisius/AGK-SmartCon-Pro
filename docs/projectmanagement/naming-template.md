...
| Режим | Описание | Пример списка | Пример валидного значения |
|---|---|---|---|
| `None` | Без ограничений | — | любое |
| `AllowedValues` | Точное совпадение с одним из значений | `["S1", "S2"]` | `S1` |
| `Contains` | Значение содержит хотя бы одну подстроку | `["AR", "ME"]` | `AR-MEP-01` |
| `CharCount` | Длина в диапазоне Min..Max | — | `ABC` при min=1, max=5 |

> Регистр не важен. Пробелы по краям значения и подстрок игнорируются.
