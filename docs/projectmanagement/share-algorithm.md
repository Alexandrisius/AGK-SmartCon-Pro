...
### Шаг 2a (только НЕ-workshared): Сохранение оригинала перед закрытием

```
2a.1. originalDoc.Save()
      - ОБЯЗАТЕЛЬНО перед Close(false): share закрывает документ пользователя
        и переоткрывает его с диска — без Save() все несохранённые изменения
        молча теряются (и не попадают в shared-копию).
      - Workshared-ветка делает эквивалент через SynchronizeWithCentral
        (SaveLocalAfter = true), поэтому шаг нужен только для локальных .rvt.
2a.2. При ошибке сохранения (read-only, диск) → abort ДО закрытия документа:
      отписка от handlers, TaskDialog с ошибкой, return Failed — документ
      пользователя остаётся открытым со всеми изменениями.
```
