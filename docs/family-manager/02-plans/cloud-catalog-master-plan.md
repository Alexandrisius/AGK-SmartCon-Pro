...
```jsonc
{
  ...
  "items": [{
    ...
    "categoryPath": "Трубопроводы/Отводы",     // путь, не id — переносимо
    ...
    "versions": [{
      ...
      "nestedSharedFamilies": [ { "name": "Болт М12" } ],  // V13, workaround REVIT-198137 — applier не может восстановить без Revit
      "file": { "sha256": "…", "sizeBytes": 843210, "fileName": "Отвод 90°.rfa" },
      "types": [ { "typeName": "ДУ50", "parameters": [ /* extracted values, storage-type-typed */ ] } ],
      "facts": [ { "key": "PartType", "valueKey": "23" } ],
...
1. **Создание базы:** диалог создания получает третий вид — **«Облачная база»** с двумя путями мастера:
   - «Опубликовать новую» = создать локальную БД + мастер первой публикации (логин в аккаунт, имя/описание/видимость/платность). **Два режима первой публикации (решение владельца 2026-09-11):** (а) полная — выложить текущее содержимое базы сразу; (б) **«пустая облачная база»** — зарегистрировать серверный каталог и `CloudLink` **без публикации контента**: работа как с обычной локальной базой (импорт семейств по одному), публикация накопленного — командой «Опубликовать изменения» (§7.3.12). Режим (б) дешевле для старта (не заливать гигабайты разом) и предпочтителен для знакомства; для среза v1 реализуется первым;
   - «Подключить существующую» = **вставка строки-приглашения** (единое поле; формат ниже) → subscribe → первый pull. Ручной ввод (endpoint/slug/ключ отдельно) — fallback; endpoint по умолчанию = официальный сервер.
...
12. **Семантика кнопок и команд по роли (решение владельца 2026-09-11):** «Обновить» на любой базе — только pull (см. §7.3.3). Публикация — отдельная явная команда **«Опубликовать изменения»** (ПКМ на облачной Published-базе + меню инструментов базы; иконка/тултип «опубликовать на сервер»): выполняет протокол pull-before-push из ADR-077 §2 — сначала подтянуть чужие публикации (мульти-автор), затем отправить свои локальные изменения (активные версии, §5); modeless-прогресс и итоговый отчёт — §7.3.9. Модель = Revit worksharing: доработал локально до стабильного → опубликовал. Автопубликация по расписанию/при импорте — отсутствует (PAT для ночной публикации — автоматизация вне UI, ADR-076 §2). Значки-облачка: базы с `CloudLink` помечаются в списке (различимые для Published и Subscribed — «облако-стрелка вверх» / «облако-стрелка вниз»), рядом с бейджем обновлений; иконки/тултипы ролей различны (пользователь может иметь обе роли на разных базах). Бейджи обновлений агрегируются per-база, без сводного счётчика.
