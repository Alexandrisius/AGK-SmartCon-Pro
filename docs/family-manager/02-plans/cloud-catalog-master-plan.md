...
Компания «ВентПроект»: BIM-мастер ведёт локальную базу арматуры → регистрирует аккаунт SmartCon Cloud → «Опубликовать базу» → генерирует ключи и рассылает **приглашения** (одна строка: endpoint+slug+ключ) 15 инженерам → каждый инженер: «Подключить облачный каталог» → регистрация аккаунта (email+пароль, consent) → вставка приглашения → первый pull → каталог в панели, семейства грузятся в проекты → BIM-мастер обновляет отвод → «Синхронизировать» → у всех бейдж «Доступны обновления» → «Обновить» → докачались 2 МБ вместо 1 ГБ. Два BIM-мастера правят разные семейства одновременно — обе правки публикуются без конфликтов (ADR-077).
...
1. **Создание базы:** диалог создания получает третий вид — **«Облачная база»** с двумя путями мастера:
   - «Опубликовать новую» = создать локальную БД + мастер первой публикации (логин в аккаунт, имя/описание/видимость/платность);
...
3. **«Обновить»** (существующая кнопка панели): для `CloudLink.Role=Subscribed` — `CheckUpdates` → если есть, диалог StatusNotice «Доступны обновления: 12 новых версий, 3 обновлено, 1 удалено» → pull → обычный refresh. Для `Published` — цикл «Синхронизировать» (pull→push, ADR-077 §2). Для локальных — текущее поведение без изменений.
...
12. **Семантика кнопки по роли:** на Published-базе кнопка подписана «Синхронизировать» (pull+push), на Subscribed — «Обновить» (pull), на локальной — текущее поведение; иконки/тултипы различны (пользователь может иметь обе роли на разных базах). Бейджи обновлений агрегируются per-база, без сводного счётчика.
...
15. **Retention-политика версий на каталоге** («хранить последние N версий» / «не старше года»): идея владельца 2026-08-26. По умолчанию история хранится вся (откат, stale detection, аудит; стоимость R2 ничтожна — $0.015/ГБ/мес); авто-подчистка старых версий — опция автора, post-MVP. Серверная механика уже есть (delete-version → tombstone → GC), нужен только планировщик и UI-настройка.
