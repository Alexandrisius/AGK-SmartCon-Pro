...
21. **Одинаковое имя исходной и подписной базы в списке (владелец, 2026-09-11, стресс-тест):** на одной машине могут сосуществовать исходник (Published) и подписная копия с одинаковым человеческим именем («Облачная база»); в выпадающем списке они различимы только значком (контурное облако vs облако+стрелка). Кандидаты: суффикс имени у подписной копии при регистрации («N (подписка)»), путь в тултипе, группировка по источнику. UX-решение — post-MVP, к фазе мульти-автора; поведение «обе базы видны одновременно, каждая со своей иконкой и меню» уже корректно.
22. **Удаление локальной Published-базы = orphan-каталог на сервере (владелец, 2026-09-11):** `DeleteDatabaseAsync` не смотрит на `CloudLink` — удаление опубликованной базы убирает запись реестра (вместе со ссылкой) и локальную папку, но на сервер НИЧЕГО не отправляет: каталог, publish points и CAS-объекты живут дальше, подписчики продолжают pull. Хуже: локальная копия была единственным носителем связи (`remote_source_json` удалён вместе с папкой) → каталог становится **осиротевшим** — повторно привязать его с этой машины нельзя, slug занят навсегда (новая база с тем же именем получит `-2`). Грамотный цикл уже описан планом: «Снять с публикации» (§7.3.2) → `DELETE /v1/catalogs/{slug}` → sunsetting 30 дней (E6: финальный pull подписчиков → 410 → GC манифестов/CAS) → у автора база конвертируется в обычную локальную. До реализации этой команды минимум-гейт для среза: подтверждение при удалении базы с `CloudLink.Role=Published` — «локальная копия будет удалена, публикация {slug} останется на сервере для подписчиков; восстановить связь будет нельзя. Сначала снимите базу с публикации» (+ команды DELETE endpoint и «Снять с публикации» — post-MVP, полный C3). Восстановление orphan-каталогов (перепривязка по slug владельцем сервера) — вместе с C3.
