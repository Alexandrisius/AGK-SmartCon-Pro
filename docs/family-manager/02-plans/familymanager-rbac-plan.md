...
## 0. Контекст для реализующего агента

### Что такое SmartCon

SmartCon — opensource плагин для Autodesk Revit (2019-2026). Флагманский модуль — PipeConnect (соединение труб). Solution: `src/SmartCon.sln`, стек: .NET 8 / C# 12 / WPF / CommunityToolkit.Mvvm.

### Что такое FamilyManager

Модуль управления библиотекой семейств Revit (.rfa). Представляет собой **dockable panel** внутри Revit с SQLite-каталогом. Пользователи могут:
- Просматривать дерево категорий и семейств
- Искать семейства по имени/тегам
- Загружать семейства в текущий проект Revit
- Импортировать новые .rfa файлы в каталог (создание версий)
- Редактировать метаданные, управлять категориями, ассетами

### Архитектура FamilyManager (текущая)

```
Данные:
  %APPDATA%\SmartCon\FamilyManager\
    registry.json              — реестр подключений к БД (какие БД известны)
    databases\
      {id}\                    — папка одной БД
        catalog.db             — SQLite (schema v6, 17 таблиц)
        storage\               — managed .rfa файлы (read-only после импорта)

Multi-DB паттерн:
  Пользователь может создать несколько БД и переключаться между ними.
  DatabaseManager управляет реестром (registry.json) и переключением.
  LocalCatalogDatabase держит одну активную SQLite-строку подключения.

SQLite:
  WAL mode, synchronous=NORMAL, busy_timeout=5000
  Один writer одновременно (ограничение SQLite)
  Типичный сценарий: 1-2 writer (BIM-мастера), 50-100 reader (инженеры)

Слои:
  SmartCon.Core         — модели (record-типы), интерфейсы сервисов
  SmartCon.Revit        — реализации через Revit API (IRevitContext, транзакции)
  SmartCon.FamilyManager — ViewModels, Views (dockable panel), SQLite-репозитории
  SmartCon.App          — DI-регистрация (ServiceRegistrar.cs), Ribbon
```

### Что СЕЙЧАС нет

В модуле **нет концепции пользователей, ролей или разрешений**. Все подключившиеся к БД имеют полный доступ. `PublishedBy` в `FamilyCatalogItem` — просто текстовое поле (кто опубликовал), не используется для авторизации.

### UI-паттерны проекта

- Все ViewModel наследуют `ObservableObject` (CommunityToolkit.Mvvm)
- Свойства: `[ObservableProperty]`, команды: `[RelayCommand]`
- Диалоги: `DialogWindowBase` (Topmost, CenterScreen, стандартный Chrome Windows)
- Стили: `ui:SingletonResources` → `Generic.xaml` из SmartCon.UI
- Кнопки: `PrimaryButton` (синяя), `SecondaryButton` (outline), `GhostButton` (прозрачная)
- Цвета: AccentBrush (#2979FF), SurfaceBrush (White), BackgroundBrush (#F5F5F5), TextPrimaryBrush (#333)

### Обязательные инварианты

Перед реализацией агент **обязан** прочитать `docs/invariants.md`. Ключевые для этой задачи:
- **I-01:** Revit API из WPF — только через `IExternalEventHandler`
- **I-03:** Транзакции — только через `ITransactionService`
- **I-09:** `SmartCon.Core` не вызывает Revit API (только типы-carriers)
- **I-10:** MVVM строго. `.xaml.cs` = только `DataContext = viewModel`
- **I-14:** Все SQLite-операции через `LocalCatalogDatabase`, `new SqliteConnection()` вне него запрещён

---

## 1. Бизнес-задача

Текущая реализация **однопользовательская**: все подключившиеся к БД имеют полный доступ (импорт, редактирование, удаление, управление категориями).
...
- 1-2 пользователя (BIM-мастера) на запись и 50-100 пользователей (Инженеров) на чтение БД
- Owner может передать владение BimMaster-у (кейс увольнения сотрудника)
...
### 2.4. Ban vs Remove — критическое различие

Owner имеет **два отдельных действия** над пользователем:

| Действие | SQL-эффект | Может переподключиться? | UI-кнопка |
|---|---|---|---|
| **Заблокировать (Ban)** | `UPDATE status = 'Banned'` (запись остаётся) | **НЕТ** — `GetOrCreateUserAsync` бросит `DbAccessDeniedException` | ToggleButton (замок) |
| **Удалить (Remove)** | `DELETE FROM db_users` (запись удаляется) | **ДА** — auto-register создаст новую запись с ролью Engineer | GhostButton (корзина) |

**Важно:** Remove ≠ Ban. Remove полезен для очистки списка (сотрудник уволился, не нужен в списке). Ban — для запрета доступа. Owner видит предупреждение при Remove: «Пользователь сможет подключиться заново как Инженер».

### 2.5. Передача владения (Ownership Transfer)

Owner может передать роль Owner другому пользователю из списка:
- Owner выбирает пользователя в Profile → нажимает «Передать владение»
- Подтверждение: «Вы станете BIM-Мастером. [User] станет Владельцем. Продолжить?»
- SQL: UPDATE `role = 'Owner'` для выбранного, `role = 'BimMaster'` для себя
- UPDATE `owner_identity` в `database_meta`
- Единственный Owner в системе не может быть удалён или заблокирован
...
`TransferOwnershipAsync` — атомарная операция: в одной транзакции UPDATE role нового Owner + UPDATE role старого Owner → BimMaster + UPDATE `database_meta.owner_identity`.
...
3. Если найден и `status = Active` → UPDATE `last_seen_at_utc` + `display_name` (если изменился DisplayName в Revit), вернуть DbUser
4. Если не найден → INSERT с `role = Engineer, status = Active`

**Почему обновлять DisplayName:** Пользователь мог сменить имя в настройках Revit. При каждом подключении обновляем `display_name`, чтобы список пользователей в Profile всегда был актуальным. Сравнение: `WHERE user_id = @id AND display_name != @newName`.
...
- Кнопка бана: `Visibility=Collapsed`
- Маленькая иконка короны (★) рядом с DisplayName

**Кнопка «Передать владение» (только для текущего Owner):**
- Расположена под заголовком «Пользователи базы» (Row 1), прижата вправо
- SecondaryButton с иконкой → открывает диалог выбора пользователя (BimMaster из списка)
- После подтверждения: текущий Owner → BimMaster, выбранный → Owner, обновляется `database_meta.owner_identity`
...
| Команда | Действие |
|---|---|
| `ChangeRoleCommand` | `IDbUserRepository.UpdateUserRoleAsync` — мгновенное применение |
| `ToggleBanCommand` | Переключает `Active ↔ Banned` через `UpdateUserStatusAsync` |
| `DeleteUserCommand` | Подтверждение «Пользователь сможет подключиться заново» + `RemoveUserAsync` |
| `TransferOwnershipCommand` | Подтверждение «Вы станете BIM-Мастером» + UPDATE owner + UPDATE self role |
...
2. **Единственный Owner**: нельзя понизить роль, удалить или забанить; Owner не может забанить сам себя
3. **Заблокированный пользователь (Banned)**: при подключении — диалог «Доступ запрещён», соединение НЕ создаётся в реестре пользователя
4. **Auto-register заблокированного**: `GetOrCreateUserAsync` бросает `DbAccessDeniedException` если `status = Banned`
5. **Multi-DB**: каждая БД — своя таблица `db_users`, независимые права. Один пользователь может быть Owner в одной БД и Engineer в другой
...
10. **Remove vs Ban**: Remove (DELETE) — пользователь может переподключиться как Engineer. Ban (UPDATE status) — пользователь не может переподключиться. Owner видит предупреждение при Remove
11. **Ownership Transfer**: Owner передаёт роль Owner → сам становится BimMaster. Подтверждение через диалог. `owner_identity` в `database_meta` обновляется
12. **DisplayName изменился**: при каждом подключении `display_name` обновляется из Revit `Application.Username`, чтобы Profile показывал актуальные имена
13. **Сетевая БД (SMB)**: SQLite WAL mode работает корректно при паттерне 1-2 writer + 50-100 reader на Windows SMB. Дополнительные меры не требуются
...
| **P3** | Сервисы (identity, repository, access control) | `RevitUserIdentityService.cs`, `LocalDbUserRepository.cs`, `DbAccessControlService.cs` | 1 день |
| **P4** | DI + DatabaseManager интеграция | `ServiceRegistrar.cs`, `DatabaseManager.cs`, `FamilyManagerViewModelFactory.cs` | 0.5 дня |
| **P5** | ViewModel: CanExecute + Profile + TransferOwnership | `FamilyManagerMainViewModel.cs`, `ProfileViewModel.cs`, `DbUserItem.cs` | 1.5 дня |
| **P6** | UI: Toolbar + Profile Dialog | `FamilyManagerPaneControl.xaml`, `ProfileView.xaml(.cs)` | 1.5 дня |
| **P7** | Тесты | `DbAccessControlServiceTests`, `LocalDbUserRepositoryTests`, `ProfileViewModelTests` | 1 день |
