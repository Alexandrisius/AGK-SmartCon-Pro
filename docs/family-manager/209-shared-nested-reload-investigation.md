...
> **Корректировка 2026-08-13 (дрейф фикстуры):** ручной тест владельца
> 2026-08-12 переписал v2-файл гайки в тестовой библиотеке (update-flow
> сохранил merge-артефакт; появился v3), и выяснилось, что «стена» сырого
> reload'а (rounds 1-5: success без замены дефиниции) была специфична к
> контенту, отличающемуся ТОЛЬКО группой параметров: с существенно
> отличающимся входящим файлом даже RAW unpoked reload приземляет v2.
> Константные пины в `RealLibraryNestedReloadReproTests` заменены живыми
> хэшами из файлов (Seed), wall-тест переработан в
> `ReloadNestedV2_IntoRealFlangePair_RawReload_DecisiveOutcome` — ассерт
> «исход однозначен» (LANDED v2 | WALL, но никогда PARTIAL MERGE) +
> precondition «embedded == v1» как детектор дрейфа фикстуры.
