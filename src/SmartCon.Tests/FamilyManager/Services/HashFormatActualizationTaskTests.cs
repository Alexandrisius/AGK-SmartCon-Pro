...
/// #159; FHV7 — #215): stale-hash detection (fmt NULL/not in {8,-1,-2} →
/// recompute to FHV8) across BOTH loadable and system sources, hash apply
...
public sealed class HashFormatActualizationTaskTests : IDisposable
{
    ...
    public async Task CountPending_CountsLoadableLegacyAndSystem_SkipsTerminalCurrentAndNewer()
    {
        ...
        await CatalogSeedHelper.SeedBareLoadableAsync(_fixture, "StaleV2", hashFormatVersion: 2);   // v2 is stale for v8
        await CatalogSeedHelper.SeedBareLoadableAsync(_fixture, "Current", hashFormatVersion: 8);
    ...
    public async Task Apply_ActiveLabel_WritesHashToAllVariants_AndSyncsItem()
    {
        ...
        Assert.Equal(8, item?.HashFormatVersion);
    }
