...
/// #159; FHV7 — #215): stale-hash detection (fmt NULL/not in {10,-1,-2} →
/// recompute to FHV10) across BOTH loadable and system sources, hash apply
...
public sealed class HashFormatActualizationTaskTests : IDisposable
{
    ...
    public async Task CountPending_CountsLoadableLegacyAndSystem_SkipsTerminalCurrentAndNewer()
    {
        ...
        await CatalogSeedHelper.SeedBareLoadableAsync(_fixture, "StaleV2", hashFormatVersion: 2);   // v2 is stale for v10
        await CatalogSeedHelper.SeedBareLoadableAsync(_fixture, "Current", hashFormatVersion: FamilyContentHashFormat.CurrentVersion);
    ...
    public async Task Apply_ActiveLabel_WritesHashToAllVariants_AndSyncsItem()
    {
        ...
        foreach (var vid in new[] { v2025, v2021 })
        {
            ...
            Assert.Equal(FamilyContentHashFormat.CurrentVersion, fmt);
    ...
    {
        // #249 follow-up: hash-v16 writes section_hashes/section_strings in
